...
class UserServiceImp extends UserService {
  ...
  @override
  Future<List<UserEntity>> getFollowingsList(String userId) async {
    final snapshot =
        await firebaseFirestore.collection('users').doc(userId).get();
    final data = snapshot.data();
    final followings = data?['followings'] as List<dynamic>? ?? [];
    final followingsList = <UserEntity>[];

    for (final followingId in followings) {
      final userSnapshot =
          await firebaseFirestore.collection('users').doc(followingId).get();
      final user = UserModel.fromJson(userSnapshot.data() ?? {});
      followingsList.add(UserEntity.fromModel(user));
    }
...
//       String userId, List<String> newFollowers) async {
//     final userDocRef = firebaseFirestore.collection('users').doc(userId);
//     await userDocRef.update({'followers': FieldValue.arrayUnion(newFollowers)});
//   }
...
//     final userDocRef = firebaseFirestore.collection('users').doc(userId);
//     await userDocRef
//         .update({'followings': FieldValue.arrayUnion(newFollowings)});
//   }
